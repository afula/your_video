...
impl Component for Media {
	...
	fn view(&self) -> Html {
		match &self.media_type {
			Type::Audio(_) => {
				html!(<audio  class=&self.class controls=true src=self.src.clone() onended=self.link.callback(|_| Msg::Ended) onseeked=self.link.callback(|_| Msg::Seeked) />)
			},
			Type::Image(_) => {
			...
			Type::Video(_) => {
				html!(
					<video  class=&self.class controls=true src=self.src.clone() onended=self.link.callback(|_| Msg::Ended) onseeked=self.link.callback(|_| Msg::Seeked) />
				)
			},
