...
use js_sys::Promise;
use web_sys::{AudioContext, Navigator, MediaDevices, MediaRecorder, MediaStreamConstraints, MediaStream};
...
pub async fn get_media()-> Result<MediaStream, JsValue> {
	//打开麦克风录音
	// console::log_1(&"获取media".into());
	let window = web_sys::window().expect("Missing Window");
	let navigator = window.navigator();
	let mediaDevice = navigator.media_devices().unwrap();
	...
	let promise  = mediaDevice.get_user_media_with_constraints(&media_constraints).unwrap();
		...
		match msg {
			Msg::StartRecorder =>{
				...
				spawn_local(async move {
					...
					let media_recorder = web_sys::MediaRecorder::new_with_media_stream(&media_stream).unwrap();
					media_recorder.start();
					// console::log_1(&payload.into());
				});
				console::log_1(&"打印log".into());
				true
			},
			Msg::StopRecorder =>{
				//暂停录音
			...
			Some(files) => {
				...
				html! {
					<div class="bg-black text-white absolute inset-0 flex place-content-center place-items-center">
					    ...
					    <div> <button style="margin-left:5px" onclick=self.link.callback(|_| Msg::StartRecorder)>{"录音"}</button></div>
				    	<div> <button style="margin-left:10px" onclick=self.link.callback(|_| Msg::StopRecorder)>{"暂停"}</button></div>
					    <div> <button style="margin-left:10px" onclick=self.link.callback(|_| Msg::GetRecorderData)>{"获取音频"}</button></div>
			...
			None => {
				...
				html! {
					<div class="bg-gray-700 text-white absolute inset-0 flex flex-col place-content-center place-items-center select-none">
						<h1 class="animate-bounce text-9xl m-2">{ "Myway" }</h1>
				    	...
				    	<input ref=self.node_ref.clone() id="directory" type="file" accept="video/*"  class="hidden" onchange=self.link.callback(move |value| {
